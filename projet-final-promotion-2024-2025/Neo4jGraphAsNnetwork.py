    ...
    def create_network(tx, network_structure, task_type, hidden_activation="relu", output_activation=None):
        ...
        for layer_index, num_neurons in enumerate(network_structure):
            ...
            for neuron_index in range(num_neurons):
                ...
                # Create neuron using stored procedure
                tx.run(
        ...
        for layer_index in range(len(network_structure) - 1):
            ...
            for i in range(num_neurons_current):
                for j in range(num_neurons_next):
                    ...
                    # Use correct procedure name
                    tx.run(
                        "CALL nn.createRelationShipsNeuron($from_id, $to_id, $weight)",
                        from_id=f"{layer_index}-{i}",
                        to_id=f"{layer_index + 1}-{j}",
                        weight=weight
                    )
...
# Update the csv_file_path in your main code
...
if __name__ == "__main__":
    ...
    try:
        ...
        network_structure = [108, 10, 3]  # Changed output layer to 3 nodes for H/D/A
        hidden_activation = "tanh"
        output_activation = "softmax"  # Changed to softmax for classification
        task_type = "classification"  # Changed to classification since FTR is categorical

        epochs = 500
        learning_rate = 0.001  # Slightly increased for better learning
        beta1 = 0.9
        beta2 = 0.999
        epsilon = 1e-8
        ...
        file_path = Path("/Users/ecembayindir/Desktop/SORBONNE/Classes/Neo4j_Cloud_2/Neo4j Project/neo4j-cours/projet-final-promotion-2024-2025/test_cases.json")
